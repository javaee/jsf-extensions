...
alert("parse flags.valiate " + flags.validate);
	if (flags.validate && !dojo.i18n.number.isReal(value, locale, flags)) {
    alert("parse2 " + dojo.i18n.number.isReal(value, locale, flags));
		return Number.NaN;
	}

	var numbers = value.split(flags.decimal);
    alert("return: " + numbers);
...
dojo.i18n.number.isReal = function(value, locale /*optional*/, flags /*optional*/) {
	...
	var re = new RegExp("^" + dojo.regexp.realNumber() + "$");
    //var re = new RegExp("^" + dojo.regexp.integer() + "$");
    alert("re=" + value);
    alert("re=" + re.test(value));
	return re.test(value);
};
