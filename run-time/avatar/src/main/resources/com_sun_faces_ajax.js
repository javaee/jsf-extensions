...
DynaFaces.gSkipUpdate = DynaFaces.gFacesPrefix + "SkipUpdate";
...
Object.extend(Object.extend(Faces.Event.prototype, Ajax.Request.prototype), {
    initialize: function(source, options) {
        ...
        var inputs = null;
        if (this.options.inputs) {
            inputs = this.options.inputs.split(",");
        }
        if (!source && !this.options.form && inputs && inputs.length > 0) {
            this.form = $Form(inputs[0]);
        } else {
            this.form = $Form(source) || $(this.options.form);
        }
        ...
        if (this.options.skipUpdate) {
            this.options.requestHeaders.push(DynaFaces.gSkipUpdate);
            this.options.requestHeaders.push("true");
        }
    ...
    renderView: function(xjson) {
        ...
        for (var i = 0; i < render.length; i++) {
            ...
            if (this.options.replaceElement) {
                ...
                var type = render[i].getAttribute('type');
                if (!type) {
                    Element.replace(id, str);
                } else {
                    var clientHandler = eval("(" + type + ")");
                    clientHandler.handle(id, str, render[i]);
                }
            }
