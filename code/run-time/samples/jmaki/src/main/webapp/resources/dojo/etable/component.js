	...
	fillCell: function(/* HTMLTableCell */cell, /* object */meta, /* object */val, /*editable*/editable, row, column){
		...
		//	Fill the passed cell with value, based on the passed meta object.
			...
			if(meta.getType()==Date) {
				...
				if(!isNaN(val)){
					...
					_val = dojo.date.strftime(val, format);
				} else {
					_val = val;
				}
            ...
            dojo.event.connect(formElement, "onsubmit", this, "updateCell");
            ...
            dojo.event.connect(cell, "ondblclick", this, "makeCellEditable");
		}
	},
        makeCellEditable : function(e) {
            ...
            if (cell.nodeName.toLowerCase().indexOf("div") != -1) {
                ...
                dojo.event.disconnect(cell, "ondblclick",this, "makeCellEditable");
...
jmaki.widgets.dojo.etable.Widget = function(wargs) {
  ...
  this.init = function() {
	  if (!auto) {
	    ...
	    table = dojo.widget.createWidget("EditableFilteringTable",{valueField: "Id"},container);
	  ...
	  dojo.event.connect(table, "onCellUpdate", self, "onCellUpdate"); 
  ...
  this.onCellUpdate = function(column,row,value){
	  // publish the event
	  jmaki.publish(topic, {src: uuid, column:column, row: row, value: value});
  }
