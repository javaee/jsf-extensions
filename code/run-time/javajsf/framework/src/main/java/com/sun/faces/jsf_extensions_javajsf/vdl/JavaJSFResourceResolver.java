...
public class JavaJSFResourceResolver extends ResourceResolver {
    ...
    public JavaJSFResourceResolver(ResourceResolver wrapped) {
    ...
    public URL resolveUrl(String path) {
        URL result = null;
        if (path.contains("javajsf")) {
            // load the template from the classpath
            ClassLoader loader = getCurrentLoader(this);
            result = loader.getResource("META-INF" + path);
            if (result == null) {
                // try using this class' loader (necessary when running in OSGi)
                result = this.getClass().getClassLoader().getResource(path);
            }
        } else {
            result = wrapped.resolveUrl(path);
        }
